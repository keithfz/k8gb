...
func TestFailoverPlayground(t *testing.T) {
	...
	actAndAssert := func(test, geoTag string, localTargets []string) {
		...
		fmt.Printf("XXXX http result instance eu: %v", httpResult)
		assert.Equal(t, geoTag, httpResult.Message)
		httpResult = instanceUS.HitTestApp()
		fmt.Printf("XXXX http result instance us: %v", httpResult)
		assert.Equal(t, geoTag, httpResult.Message)
	}
	...
	fmt.Printf("XXXX eu %v, us: %v", euLocalTargets, usLocalTargets)

	t.Run("stop podinfo on eu cluster", func(t *testing.T) {
		fmt.Printf("XXXX stop eu app  eu %v, us: %v", euLocalTargets, usLocalTargets)
		instanceEU.StopTestApp()
		require.NoError(t, instanceEU.WaitForAppIsStopped())
		fmt.Printf("XXXX app stopped eu %v, us: %v", euLocalTargets, usLocalTargets)
