...
// WaitForLocalTargets returns instance local targets
func (i *Instance) WaitForLocalTargets() []string {
	const maxRetries = 20
	for d := 0; d < maxRetries; d++ {
		targets := i.GetLocalTargets()
		if len(targets) > 0 {
			return targets
		}
